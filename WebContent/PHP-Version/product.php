...
EOL;
  ...
  if($row = $stmt->fetch()) {
    if($row['productImage']!=null && $row['productImage']!="")
      echo "<img src=\"displayImage.php?id=$productId\" height=\"533\" width=\"300\">";
    if($row['productImageURL']!=null && $row['productImageURL']!="")
      echo "<img src=\"$row[productImageURL]\">";
    echo <<< EOL
    <br><a href="addcart.php?id=$productId&name=$row[productName]
...
<table>
  <thead>
    <tr><th colspan="2"><?php echo $row[productName];?></th></tr>
  </thead>
  <tbody>
...
<table class="table table-hover table-responsive-md">
        ...
        echo "<h1>No product found (if)!";
      }
    } catch(\PDOException $e) {
      echo "<h1>No product found (try)!</h1>";
    }
      ?>
			<br>
		</div>
